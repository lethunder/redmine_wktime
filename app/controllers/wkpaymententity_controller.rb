		...
		respond_to do |format|
			...
			format.csv do
				...
				data = entries.map do |e|
					...
					{ payment: e.id, type: personTypeLabelHash[e.entity_type], name: e&.parent&.name,  payment_date: e.payment_date.strftime("%Y-%m-%d"), payment_type: getPayTypeHash[e.payment_type_id], original_amount: (payment_items&.original_currency || '')+" "+e.payment_original_amount.round(2).to_s, amount: (payment_items&.currency || '')+" "+e.payment_amount.round(2).to_s }
				end
				respond_to do |format|
					format.csv {
